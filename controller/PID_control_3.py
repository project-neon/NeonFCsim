...
class PID_control_3(object):
    def __init__(self, robot, default_fps=60, max_speed=1.8, max_angular=2400, krho=100,kb=0, kp=60, ki=0, kd=0, reduce_speed=False, two_sides=False, spread=0):
        ...
        self.KB = kb # Proportional gain of w in angular error.
    ...
    def update(self):
        ...
        if self.robot.team_color == "yellow_team":
            self.desired_angle += np.pi
        ...
        # BETA angle between the front of the robot and the desired angle
        beta = angle_adjustment(self.desired_angle - self.robot.theta)
        ...
        """Angular speed (w)"""
        w = self.KB * beta + self.KP * alpha + self.KI * self.int_alpha + self.KD * self.dif_alpha
